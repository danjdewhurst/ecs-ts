import { World, BaseSystem, type Component } from '../src/index.ts';
    ...
    update(world: World, deltaTime: number): void {
        this.queryWithComponents<HealthComponent>(world, 'health', (entityId, health) => {
            if (health.hp < health.maxHp) {
                health.hp = Math.min(health.maxHp, health.hp + (10 * deltaTime)); // Regen 10 HP per second
...
function runExample() {
    ...
    console.log(`Entities with health component: [${healthQuery.getEntities().join(', ')}]`);

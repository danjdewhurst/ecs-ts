import { World, BaseSystem, EventComponent, type Component, type GameEvent } from '../src/index.ts';
...
class CombatSystem extends BaseSystem {
    ...
    constructor(world: World) {
        ...
        this.unsubscribeFromDamage = world.subscribeToEvent('damage', (event) => {
    ...
    private handleDamageEvent(event: GameEvent, world: World): void {
        ...
        if (typeof entityId === 'number' && typeof damage === 'number') {
            ...
            if (health) {
                ...
                console.log(`Entity ${entityId} took ${damage} damage, health: ${health.current}/${health.maximum}`);
...
class DeathSystem extends BaseSystem {
    ...
    constructor(world: World) {
        super();
        this.unsubscribeFromDeath = world.subscribeToEvent('entity-death', (event) => {
...
class EffectSystem extends BaseSystem {
    ...
    constructor(world: World) {
        super();
        this.unsubscribeFromEffects = world.subscribeToEvent('death-effect', (event) => {
    ...
    private handleEffectEvent(event: GameEvent, world: World): void {
        const { entityId, effectType, position } = event.data;
        const pos = position as Position | undefined;
        
        console.log(`✨ Creating ${effectType} effect at position (${pos?.x ?? 0}, ${pos?.y ?? 0}) for entity ${entityId}`);
    }
...
function runEventSystemExample(): void {
    ...
    world.addComponent<Health>(warrior, { type: 'health', current: 100, maximum: 100 });
    
    const mage = world.createEntity();
    world.addComponent<Position>(mage, { type: 'position', x: 200, y: 75 });
    world.addComponent<Health>(mage, { type: 'health', current: 50, maximum: 50 });
    ...
    world.subscribeToEvent('victory', (event) => {
        console.log(`🎉 Victory Event: ${event.data.message} (XP: ${event.data.experienceGained})`);
    ...
    console.log(`Event listeners: ${world.getEventBus().getListenerCount('damage')} damage listeners`);

...
describe('World', () => {
    ...
    test('should throw error when adding component to non-existent entity', () => {
        ...
        expect(() => world.addComponent(999, position)).toThrow('Entity 999 does not exist');
    });
    ...
    test('should provide component type statistics', () => {
        ...
        expect(archetypeStats.find(s => s.archetype === 'position')?.entityCount).toBe(1);
        expect(archetypeStats.find(s => s.archetype === 'health|position')?.entityCount).toBe(1);
    });
});